...
namespace QrCodeDetector
{
    public sealed partial class MainPage : Page
    {
        ...
        BarcodeReader m_reader = new BarcodeReader
        {
            ...
            {
                PossibleFormats = new BarcodeFormat[] { BarcodeFormat.All_1D, BarcodeFormat.QR_CODE},
                //code128 22 digit Barcode. 
                TryHarder = true
            }
        };
        ...
        private async Task InitializeCaptureAsync()
        {
            ...
            catch (Exception e)
            {
                ...
                ///TextLog.Text = String.Format("Failed to start the camera: {0}", e.ToString());
            }
        ...
        private void AnalyzeBitmap(Bitmap bitmap, TimeSpan time)
        {
            ...
            var ignore = Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                ...
                else
                {
                    ...
                    App.localSettings.Values["barcode"] = "found";
                    if(result.Text.Length==22)
                    {
                        Debug.WriteLine("result"+result.Text);
                        App.localSettings.Values["bar_value"] = result.Text.Substring(3, 13);
                        Debug.WriteLine("inside barcode" + App.localSettings.Values["bar_value"]);
                    }
                    else if (result.Text.Length == 12)
                    {
                        App.localSettings.Values["bar_value"] = "0" + result.Text;
                    }
                    else
                    {
                        App.localSettings.Values["bar_value"] = result.Text;
                    }
                    Debug.WriteLine("barcode text"+App.localSettings.Values["bar_value"]);
                    App.localSettings.Values["type"] = result.BarcodeFormat.ToString();
                    Debug.WriteLine("Barcode type"+App.localSettings.Values["type"]);
                   // App.localSettings.Values["type"] = result.Type;
                    if(dupFlag++==0)
